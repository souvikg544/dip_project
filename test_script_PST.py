...
from PST_function1 import PST

# [] To process high resolution images set 
# from PIL import Image
# Image.MAX_IMAGE_PIXELS = 1000000000
# Replace cv2.imread by Image.open
...
Image_orig = cv2.imread(filepath) # Read the image.
# To convert the color image to grayscale
if Image_orig.ndim ==3:
    Image_orig_grey = cv2.cvtColor(Image_orig, cv2.COLOR_BGR2GRAY )  # Image_orig is color image.
else: 
...
if Morph_flag ==0:
    ...
    cv2.imsave(output_path, Edge)
   
else:
    Overlay=cv2.overlay(Image_orig_grey,Edge)
